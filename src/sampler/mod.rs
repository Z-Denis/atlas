...
impl<P> Metropolis<P> {
    ...
    where
        ...
        S::Scalar: Clone + Element,
        P: Proposal<B, S, K>,
        F: Fn(&S, Tensor<B, 2, K>) -> Tensor<B, 1>,
    {
        ...
        let logp_current = log_density(space, current.clone());
        let logp_proposal = log_density(space, proposal.clone());
...
mod tests {
    ...
    fn metropolis_updates_chain_state() {
        ...
        let log_density =
            |space: &HomogeneousSpace<Spin>, samples: Tensor<Flex, 2, Int>| model.log_value(space, samples);
    ...
    fn local_spaces_integrate_with_sampler() {
        ...
        let spin_log_density =
            |space: &HomogeneousSpace<Spin>, samples: Tensor<Flex, 2, Int>| {
